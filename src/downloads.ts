...
export const isRequestedDownload = (
  download: Download
): download is RequestedDownload => download.status === 'requested'

export type QueuedDownload = Omit<RequestedDownload, 'status'> & {
  status: 'queued'
  queuePosition: number
}
export const isQueuedDownload = (
  download: Download
): download is QueuedDownload => download.status === 'queued'
...
export const isConnectedDownload = (
  download: Download
): download is ConnectedDownload => download.status === 'connected'

export type DownloadingDownload = Omit<ConnectedDownload, 'status'> & {
  status: 'downloading'
}
export const isDownloadingDownload = (
  download: Download
): download is DownloadingDownload => download.status === 'downloading'

export type CompleteDownload = Omit<DownloadingDownload, 'status'> & {
  status: 'complete'
}
export const isCompleteDownload = (
  download: Download
): download is CompleteDownload => download.status === 'complete'

export type DownloadWithToken =
  | ConnectedDownload
  | DownloadingDownload
  | CompleteDownload
export const downloadHasToken = (
  download: Download
): download is DownloadWithToken =>
  isConnectedDownload(download) ||
  isDownloadingDownload(download) ||
  isCompleteDownload(download)
...
export const makeDownloadStatusData = <T extends Download>(
  obj: T
): DownloadStatusData =>
